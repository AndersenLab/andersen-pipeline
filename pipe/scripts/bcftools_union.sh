...
parallel --gnu -j {cores} 'samtools mpileup -r {{}} {command_samtools.params} --fasta-ref {reference} {bam_merged} | bcftools call -T {vcf_path}/snps/merged_sites.tsv --skip-variants indels -O v {command_bcftools.params} - | tb geno het-polarization - | bcftools view -O z > {vcf_path}/snps/{SM}.{{}}.union.bcftools.vcf.gz' ::: ${{contigs}}
