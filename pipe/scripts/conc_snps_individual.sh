...
tb geno het-polarization - | \
egrep '(^#|1/1)' | \
bcftools query -f'%CHROM\t%POS\t%REF,%ALT\n' > {vcf_path}/snps_concordance/{ID}.nodp.individual.sites.tsv

bcftools concat ${{order}} -O z > {vcf_path}/snps_concordance/{ID}.individual.sites.vcf.gz
bcftools index {vcf_path}/snps_concordance/{ID}.individual.sites.vcf.gz
