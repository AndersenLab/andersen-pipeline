...
parallel --gnu -j {cores} 'samtools mpileup -r {{}} {command_samtools.params} -G <(bam readgroups {bam_merged} --include={ID}) --fasta-ref {reference} {bam_merged} | bcftools call --insert-missed -T {vcf_path}/snps_concordance/merged_sites.tsv --skip-variants indels -O v {command_bcftools.params} - | tb rename --subst={SM}:{ID} - | tb geno het-polarization - | bcftools view -O z > {vcf_path}/snps_concordance/{ID}.{{}}.union.bcftools.vcf.gz' ::: ${{contigs}}
