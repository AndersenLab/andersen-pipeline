...
parallel --gnu -j {cores} 'samtools mpileup -r {{}} {command_samtools.params} -G <(bam readgroups {bam_merged} --include={ID}) --fasta-ref {reference} {bam_merged} | bcftools call --skip-variants indels -m --gvcf 3 -T {vcf_path}/snps_concordance/merged_sites.tsv -O v - | tb rename --subst={SM}:{ID} - | tb geno het-polarization - | bcftools view -O z > {vcf_path}/snps_concordance/{ID}.{{}}.union.bcftools.vcf.gz' ::: ${{contigs}}
